...
//定义数据结构，是因为所有的RSS源网页类型的网页源代码都采用相似的文件结构DOM
type (
...
func (m rssMatcher) Search(feed *search.Feed, searchTerm string) ([]*search.Result, error) {
	...
	for _, channelItem := range document.Channel.Item {
		...
		// MatchString 检查channelItem.Title中是否存在匹配searchTerm的子序列。返回 bool,error 类型
		// 是在右键的网页源代码里面查找的，并没有经过浏览器修改后的F12的下面查找
